...
import kr.open.library.easy_extensions.result.fold
import kr.open.library.easy_extensions.result.map
import kr.open.library.easy_extensions.result.onFailure
import kr.open.library.easy_extensions.result.onSuccess
...
class MainActivity : AppCompatActivity() {
    ...
    private fun demonstrateResultExtensions() {
        ...
        val emailResult = "user@example.com".validateEmailResult()
        emailResult.onSuccess { isValid ->
            if (isValid) {
                toastShowShort("유효한 이메일 형식입니다")
            } else {
                toastShowShort("잘못된 이메일 형식입니다")
            }
        }.onFailure { exception ->
            toastShowShort("이메일 검증 오류: ${exception.message}")
        }

        // Context 작업 - Context operations
        val urlResult = openUrlResult("https://www.google.com")
        urlResult.onSuccess {
            toastShowShort("Result 패턴으로 URL 열기 성공")
        }.onFailure { exception ->
            toastShowShort("URL 열기 실패: ${exception.message}")
        }

        // 앱 설치 확인 - App installation check
        val appResult = isAppInstalledResult("com.android.chrome")
        appResult.onSuccess { isInstalled ->
            toastShowShort("Chrome 설치 상태: $isInstalled")
        }.onFailure { exception ->
            toastShowShort("앱 확인 오류: ${exception.message}")
        }

        // 복잡한 Result 체이닝 - Complex Result chaining
        val dateResult = "2023-12-25".parseDateResult("yyyy-MM-dd")
        val mappedResult =
            dateResult.map { date ->
                "날짜: ${date.time}"
            }
        mappedResult.onSuccess { formattedDate ->
            toastShowShort("날짜 파싱 및 변환 성공: $formattedDate")
        }.onFailure { exception ->
